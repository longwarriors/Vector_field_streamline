...
Chromium 只需在首次运行前端语义测试或 Playwright 版本升级后安装；命令仍由 uv 环境中的 Python Playwright 提供，不需要 Node 项目工具链。默认测试同时执行分支覆盖统计和浏览器语义测试，精确综合覆盖率不得低于 90.00%。CI 在 Ubuntu + Python 3.13 上执行权威覆盖率与浏览器门槛，并在 Ubuntu/Windows + Python 3.11/3.14 上验证非浏览器兼容性。
...
后续路线包括等通量播种与显示图层、规则/非结构网格场、FEMM/VTK 导入、真正的三维 Web 场线，以及与普通向量场分离的 Schwarzschild/Kerr 测地线后端。

点电荷和面内磁偶极子可在前端增删；磁偶极矩角度可直接编辑。客户端会在每次提交前核对播种源数，并自动抬高不足的 `density`，不会让交互用户裸撞服务端的种子预算 422。
