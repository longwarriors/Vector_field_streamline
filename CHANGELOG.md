...
- 场线图改用淡紫配色：热图色图由 viridis 换成同一色相的淡紫色阶，明度 L* 从 97 单调降到 40，场越强颜色越深；场线与方向箭头改为带白边的黑墨线。
- 场源标记改用粉（正电荷）、蓝（负电荷）、浅黄（磁偶极）、绿（电流线圈），都与淡紫热图区分得开；场源便签使用对应的浅色。
- 未着色格点仍画灰底斜线；灰底改为中性灰，比色图最深端亮得多，不会被读成最强的场。“读图与交互”便签和“已同步”状态不再使用青绿色。
