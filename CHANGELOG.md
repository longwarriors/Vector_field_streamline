...
- 场线追踪提速，输出与 v0.3.2 逐位相同（验证范围见开发指南的 v0.3.3 基准一节）：单点求值不再经过 NumPy 包装函数，事件函数复用右端函数刚算过的场值，圆环一般区域的单点求值改用 Python 浮点。端到端请求 `density=18`、`resolution=80` 的场景，圆环从约 1.48 s 降到 0.35 s，其余预设耗时减少 10–20%。
- 服务端缓存最近 32 份追踪结果，键为去掉 `resolution` 的请求；只改分辨率时不再重新追踪，一次请求约 20 ms。每个响应持有自己的场线副本，响应内容不变。
