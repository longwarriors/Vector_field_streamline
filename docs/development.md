...
|---|---:|---:|---:|
| `electric_dipole` | 240 ms | 200 ms | 20 ms |
| `magnetic_dipole` | 500 ms | 410 ms | 20 ms |
| `halbach_array` | 230 ms | 200 ms | 20 ms |
| `current_loop` | 1480 ms | 350 ms | 20 ms |
| `uniform` | 230 ms | 180 ms | 20 ms |

v0.3.2 没有曲线缓存，只改分辨率与首次请求耗时相同。首次请求的提速来自三处：去掉单点求值时 NumPy 包装函数的开销、事件函数复用右端函数刚算过的场值，以及圆环一般区域的单点浮点路径。缓存命中后的剩余耗时是标量网格采样和序列化，范围约 15–30 ms。

逐位不变的检查覆盖 64 个场景：五个预设各取 4 种预算和 3 种分辨率，另加不等强电荷、三电荷、旋转磁偶极和含零强度偶极的编辑后 Halbach 阵列。每个场景的完整响应 JSON，以及每个追踪任务的终止原因、右端函数调用次数和点数组，均与 v0.3.2 相同；同一预算的后两种分辨率走缓存命中，因此这也检查了命中后的响应。磁偶极子与点电荷剩余的主要开销是分数次幂和 `einsum`，按上面的规则留在数组路径。
