...
二维 mask 是三维 `ToroidalExclusion` 与这个子午面的精确截面，因而表现为 $x=\pm a$ 处的两个圆盘；这两个 mask 圆盘的半径只控制数值终止与显示空洞，不是物理线径。种子半径由环内赤道段上等间隔的 $\psi$ 目标求根得到，再镜像到轴线两侧，属于等通量播种，见[切片与验证](04-slices-and-validation.md)中的“等通量播种”。靠近导线的闭轨启用 `closed_loop` 后只走一周；会离开视域的轨迹从赤道正向积分，再用严格的赤道镜面对称补齐另一半，拼接后的点序仍沿 $+\mathbf B$。
