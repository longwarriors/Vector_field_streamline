...
## 界面：在不连续处停下再重启 { #interfaces }

材料边界两侧的场不连续。误差控制的求解器仍会用一步跨过去：这一步的各级导数一部分取自界面内、一部分取自界面外，折射点就被抹成一段圆弧，位置误差与步长同量级，且不会被后续步骤纠正。

`FieldLineTracer(..., interfaces=[...])` 把每个界面登记为终止事件（`direction=0`，两个方向都触发）。事件把积分停在界面上，然后追踪器沿最后一步的弦方向前进几个 ulp，直到界面的带符号距离变号，再从那里开始新的积分段；弧长参数连续，输出点序也连续。用到达界面的运动方向而不是界面上的场方向，是因为界面上一点的场可能已经属于另一侧，会指回来。重启前先检查新一侧的场：为零则以 `null_field` 终止在界面上（导体球），非有限则 `nonfinite_field`。若外侧场把线又推回界面，重启不会前进；连续三次没有进展就以 `solver_failure` 终止，并在 `message` 里注明这是沿界面的滑动接触。

事件只在积分步的端点检查符号，所以还要防止一步之内一进一出：追踪器把每段积分的长度限制为到最近界面的距离，单位速度的线走不到界面就不可能越过它，一段之内也就不可能漏掉界面；段长有 $10^{-4}\,\mathrm{max\_step}$ 的下限，比这更短的擦入弦仍可能漏检。恰在界面上的种子先沿其场方向移动几个 ulp，再开始积分。事件只把**折射点**放回界面上，跨越那一步自身的局部误差仍在容差量级。介质球预设用 $\mathbf D$ 的通量函数逐线检验：相对漂移约 $10^{-5}$，无界面事件时约 $10^{-4}$。没有登记界面时代码路径与此前逐位相同，回归测试用匀强场比较了两种调用的点数组字节。
...
圆环回归测试不用解析场自身的 $A_\phi$ 公式来“自证”：它另对 Biot–Savart 矢势线积分做 Gauss–Legendre 求积，再检查该 $\psi=\rho A_\phi$ 沿数值轨迹的相对漂移。切向残差测试把输出间距缩小四倍并要求残差至少改善十倍，符合光滑曲线中点弦近似的二阶行为。靠近导线但位于环面排除区之外的闭轨还必须以 `closed_loop` 在一周后终止，不能靠 `max_arc_length` 重复绕行。
