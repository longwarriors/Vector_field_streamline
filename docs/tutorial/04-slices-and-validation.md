...
这种“等间距”没有自动的物理权重。当前前端使用的就是覆盖播种；磁偶极与 Halbach 预设围绕每个非零偶极子的实际有符号磁矩轴分配种子，零强度偶极只保留为显示 marker，不产生场、种子或排除区域；圆环预设在环内赤道段按半径等距放种子。这些规则都用于几何覆盖，没有把等角度或等半径间距冒充等磁通。
...
不要只用含义不明的 `density` 表示播种设置。它是整数种子总预算，不是场强或“每个源”的数量。界面和导出数据还应显示 `seed_mode`；v0.2.2 保持自由文本说明，`coverage`/`equal_flux`/`feature` 枚举与独立说明字段计划在 v0.3.0 引入。
