...
当前四个浏览器预设没有使用任意平面投影：电偶极子与磁偶极子采用对称不变平面，圆形电流线圈采用含轴的 $z=0$ 子午面，匀强场则直接定义在二维平面中。`metadata.projection_note` 会明确记录切片方式。
...
这种“等间距”没有自动的物理权重。当前前端使用的就是覆盖播种；圆环预设在环内赤道段按半径等距放种子，也没有把等几何间距冒充等磁通。
