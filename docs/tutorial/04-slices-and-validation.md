...
同样的推理适用于任何轴对称、无散的场，不限于磁场。带电圆环的电场在环外无散，其通量函数 $\Psi$ 是穿过同轴圆盘的电通量除以 $2\pi$；相邻等 $\Psi$ 线之间的三维电通量仍是 $2\pi\Delta\Psi$。`charged_ring` 预设在两个截面周围的种子圆上求根：绕截面一周，$\Psi$ 从环外赤道割线上方的 $+Q/(4\pi\varepsilon_0)$ 单调降到割线下方的 $-Q/(4\pi\varepsilon_0)$，把这个区间等分并取各段中点，每条线就代表相等的通量。等通量不等于等角度：种子向环外赤道一侧密集，因为那一侧的线要覆盖更大的旋转面。目标恰为 0 的种子是环内赤道线，它通向中心鞍点，所以直接放在赤道上而不经过求根，避免舍入把它偏转到轴上。奇数预算的额外线是环外赤道射线，它分隔上下两半场，不带等通量权重。

有材料界面时还要说清“哪个场的通量”。介质球场景的 $\mathbf E$ 通量在球面不守恒，$\mathbf D$ 通量守恒，所以 `dielectric_sphere` 与 `conducting_sphere` 按 $\mathbf D/\varepsilon_0$ 的通量函数在左边界等分播种，每条线代表相等的电位移通量 $2\pi\Delta\Psi_D$，球内外的线密度可以直接比较；线穿过球面折射后仍是同一条 $\Psi_D$ 等值线。导体球把 $\Psi_D$ 小于球截面通量的线全部收进球面，这正是它把外场“吸”进来的几何表述。
