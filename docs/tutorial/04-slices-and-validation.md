...
单磁偶极预设沿偶极赤道线的两个方向按距离铺开种子并双向积分，用来揭示闭合磁力线族，所以标为 `feature`。负强度把实际磁矩反转 $180^\circ$，但赤道仍是同一条直线，种子几何不应因此变化。默认 Halbach 虽然同样在两条指定线段上播种，其目标是覆盖强弱两侧而不是追踪单个特征流形，故仍标为 `coverage`。
