...
当前五个浏览器预设没有使用任意平面投影：电偶极子、磁偶极子和 Halbach 教学阵列采用对称不变平面，圆形电流线圈采用含轴的 $z=0$ 子午面，匀强场则直接定义在二维平面中。Halbach 阵列的偶极矩全部位于 $z=0$，因此该平面上的法向分量解析地为零；`metadata.projection_note` 会明确记录切片方式。
