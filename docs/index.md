...
当前版本可以在浏览器中查看电偶极子、电四极子、六个正电荷、三对交替电荷、理想磁偶极子、线性 Halbach 阵列、圆形电流线圈、带电圆环、匀强场中的介质球与导体球，以及匀强场。点源可以增删和拖动，面内磁偶极矩还可旋转；圆环的两个截面标记则是只读几何。图中同时显示单位、枚举播种模式与具体说明、切片含义、双端终止统计、抑制数和实际渲染数；探针吸附到最近的网格点并显示未被色标上下界裁剪的原始有限场值。图框旁的工具栏可以单独开关热图、场线、方向、场源和网格五个图层，切换不重新计算。
...
浏览器不重复实现库仑场、磁偶极公式或 ODE。更换色图不需重算场，更换种子不需重建模型；源集合、位置、强度或方向角变化时，已有场值才失效。

拖动源时，新位置与旧场暂时不属于同一物理场景。界面会隐藏旧热图和场线、显示“场待重算”，并在松开后只提交一次合法最终位置；它不会把旧计算结果混画到新源上。
...
4. **斜线与空白**：图框内的灰底斜线是无法着色的格点，来自 JSON `null` 标出的奇点 mask，或对数色标无法显示的非正值；图框外的空白才是计算域之外。
