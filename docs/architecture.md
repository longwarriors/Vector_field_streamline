...
目前服务端实现的是曲线层。`scene.py` 以去掉 `resolution` 的请求 JSON 为键，在进程内保留最近使用的至多 32 份追踪结果，并以估计 64 MiB 为内存上限；一个预设场景的曲线约 0.1–0.5 MB，实际起约束作用的是条数上限。命中时直接返回同一键下算出的结果，因此响应与重新追踪完全相同。缓存随进程重启清空，所以键中不需要求解器版本。标量网格每次重新采样，耗时远小于追踪；显示状态只存在于浏览器。
